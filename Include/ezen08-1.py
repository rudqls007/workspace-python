...
# prettify() 함수는 HTML source를 tab을 기준으로 예쁘게 보여준다.
#source = source.prettify()
#print(source)

# 우리가 찾고 싶은 데이터 : 검사 기능을 통해서 td class = "number_1"에 있는 것을 확인함
# td tag를 찾아서, class가 number_1인 데이터를 모두 가지고오면 체결가를 가져온다.
# web crawling : HTML source 내에 숨어있는 데이터가 속한 tag에서 데이터를 추출하는 작업이다.


# find_all() : HTML source에서 특정 조건을 만족하는 모든 tag를 가져오는 함수
